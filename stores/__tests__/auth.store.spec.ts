...
describe("AuthStore", () => {
    ...
    describe("Login", () => {
        it('should send the user data with status `success` when the login succeed ans set the user session', async () => {
            ...
            vi.spyOn(authService, "login").mockImplementationOnce(() => Promise.resolve({
                data: successLoginData,
            }));

            const responseOk = await authStore.login("myemail@gmail.com", "myHigh@1Password");
        ...
        describe("On Error", () => {
            it('should return an error on bad request', async () => {
                ...
                const responseError = await authStore.login("myemail@gmail.com", "myHigh@1Password");
