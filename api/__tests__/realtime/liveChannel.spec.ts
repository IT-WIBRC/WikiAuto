...
import type { SupabaseChannel } from "../..";
...
describe("LiveChannelCore", () => {
  ...
  it("should call attachQueuedListeners and log onConnectionEstablished", () => {
  ...
  it("should set isConnected to false, call clearQueuedListeners, and log error onConnectionLost", () => {
  ...
  it("should call onConnectionEstablished or onConnectionLost in monitorConnectionStatus", () => {
    ...
    onConnectionEstablishedSpy.mockClear();
  ...
  it("should empty the awaitingConnection array in clearQueuedListeners", () => {
    liveChannelCore["awaitingConnection"] = [
      {
        event: "INSERT",
        handler: vi.fn(),
  ...
  it("should add a listener to awaitingConnection if not already queued", () => {
  ...
  it("should not add a listener to awaitingConnection if already queued", () => {
    const consoleWarnSpy = vi.spyOn(console, "warn");
    const mockHandler = vi.fn();
    const listener = {
      event: "INSERT",
  ...
  it("should call socket.on with correct parameters in supabaseListen", () => {
    const mockSocket = {
  ...
  it("should not subscribe if isConnected is already true", () => {
    ...
    const consoleLogSpy = vi.spyOn(console, "log");
    liveChannelCore["establishConduit"]();
    expect(mockSocket.subscribe).not.toHaveBeenCalled();
    expect(consoleLogSpy).not.toHaveBeenCalled();
  });

  it("should call supabaseListen if socket and conduit exist in attachListener", () => {
    const mockSocket = {
      on: vi.fn().mockReturnThis(),
    } as unknown as SupabaseChannel;
    liveChannelCore["socket"] = mockSocket;
  ...
  it("should queue the listener if supabaseListen returns null", () => {
    ...
    const queueListenerSpy = vi.spyOn(liveChannelCore as any, "queueListener");
    const mockHandler = vi.fn();
    liveChannelCore["attachListener"]("INSERT", mockHandler);
  ...
  it("should call conduit.channel with the channelKey and set the socket in openConduit", async () => {
    const mockChannel = {
      on: vi.fn().mockReturnThis(),
      subscribe: vi.fn().mockReturnThis(),
  ...
  it("should unsubscribe from the socket, remove the channel, and set socket to null in closeConduit", async () => {
    const mockSocket = {
      unsubscribe: vi.fn().mockResolvedValue(""),
    } as unknown as SupabaseChannel;
    liveChannelCore["socket"] = mockSocket;
  ...
  it("should handle the case where socket is null in closeConduit", async () => {
    const consoleWarnSpy = vi.spyOn(console, "warn");
    liveChannelCore["socket"] = null;
    await liveChannelCore.closeConduit();
    expect(consoleWarnSpy).toHaveBeenCalledWith(
...
describe("LiveChannel", () => {
  ...
  it("should add handler to activeHandlers and call attachListener in watch", () => {
    const attachListenerSpy = vi.spyOn(liveChannel as any, "attachListener");
    const mockHandler = vi.fn();
    liveChannel.watch("INSERT", mockHandler);
    expect(liveChannel["activeHandlers"].get("INSERT")).toContain(mockHandler);
  ...
  it("should not add duplicate handlers for the same event", () => {
    ...
    liveChannel.watch("INSERT", mockHandler);
    liveChannel.watch("INSERT", mockHandler);
    expect(liveChannel["activeHandlers"].get("INSERT")).toEqual([mockHandler]);
